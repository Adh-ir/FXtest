...
class TestDataProcessorForwardFill:
    ...
    def test_process_results_forward_fill(self):
        ...
        mock_api_data = {
            'values': [
                {'datetime': '2023-01-06', 'close': '10.0'}
            ]
        }
        ...
        with patch('logic.data_processor.datetime') as mock_dt:
        ...
        assert dates == ['2023-01-08', '2023-01-07', '2023-01-06'] # Descending order check
    ...
    def test_process_results_limit_constraint(self):
        ...
        mock_api_data = {
            'values': [
                {'datetime': '2023-01-01', 'close': '10.0'}
            ]
        }
...
class TestAuditorLookback:
    ...
    @patch('logic.auditor._fetch_rate_raw')
    def test_fetch_rate_fallback_success_immediate(self, mock_raw):
        """Test success on exact date."""
        mock_raw.return_value = 1.5
        
        rate = _fetch_rate_with_fallback('key', 'USD', 'ZAR', '2023-01-05')
        
        assert rate == 1.5
        mock_raw.assert_called_once_with('key', 'USD', 'ZAR', '2023-01-05')

    @patch('logic.auditor._fetch_rate_raw')
    def test_fetch_rate_fallback_lookback(self, mock_raw):
        ...
        mock_raw.side_effect = [None, None, 1.5]
        
        rate = _fetch_rate_with_fallback('key', 'USD', 'ZAR', '2023-01-01')
        
        assert rate == 1.5
        assert mock_raw.call_count == 3
        
    @patch('logic.auditor._fetch_rate_raw')
    def test_fetch_rate_fallback_fail(self, mock_raw):
